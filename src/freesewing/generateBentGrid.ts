// src/freesewing/generateBentGrid.ts

import fs from "fs";
import path from "path";
// @ts-ignore - ESM module resolution
...
async function main() {
  ...
  const freesewingModule = "@freesewing/bent";
  const freesewingVersion = "3.1.0";

  const patternId = "PATTERN_BENT_MEN_SLEEVE";
  const blockIdMain = "BLOCK_SLEEVE_MEN_BRIAN_BASE";
  ...
  for (const body_id of bodies) {
    for (const shape_id of shapes) {
      ...
      const meta = createSampleMeta({
        ...
        notesJa: `Bent × ${body_id} × ${shapeLabel} の自動生成サンプル。`,
      });
  ...
  console.log("🎉 Bent グリッド生成（M/L × NOSHAPE/NEKOSE）完了");
}

main().catch((err) => {
  console.error("❌ generateBentGrid でエラー:", err);
  process.exit(1);
});
